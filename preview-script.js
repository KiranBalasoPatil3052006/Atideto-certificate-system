...
async function loadStudentData(studentId) {
  ...
  try {
    ...
    $("qrLink").value = certId ? `https://atideto-certificate-system.vercel.app/studentverify.html?id=${encodeURIComponent(certId)}` : "";
...
$("generateBackendBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    resultEl.style.display = "block";
    resultEl.style.background = "rgba(0,210,106,0.1)";
    resultEl.style.color = "#00D26A";
    resultEl.innerHTML = `Certificate generated: <strong>${cert.certificateId}</strong>`;

    /* Update form with generated data */
    $("verifyId").value = cert.certificateId || "";
    $("qrLink").value = cert.verificationUrl || `https://atideto-certificate-system.vercel.app/studentverify.html?id=${encodeURIComponent(cert.certificateId)}`;
    renderAll();
    updateQr();

    /* Add download link */
    if (cert.pdfUrl) {
      resultEl.innerHTML += `<br><a href="${cert.pdfUrl}" target="_blank" style="color:#2EA8FF;">Download PDF</a>`;
    }
  } catch (err) {
    resultEl.style.display = "block";
