...
function attachLiveListeners(){
  ...
  form.addEventListener("input", (e) => {
    ...
    if(e.target.id === "verifyId" && !$("qrLink").dataset.userEdited){
      ...
      $("qrLink").value = val ? `https://atideto-certificate-system.vercel.app/?id=${encodeURIComponent(val)}` : "";
      updateQr();
    }
  });
...
async function loadStudentData(studentId) {
  ...
  try {
    ...
    $("qrLink").value = certId ? `https://atideto-certificate-system.vercel.app/?id=${encodeURIComponent(certId)}` : "";
...
$("generateBackendBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    $("qrLink").value = cert.verificationUrl || "";
    renderAll();
    updateQr();
